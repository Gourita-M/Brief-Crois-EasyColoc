...
<body class="bg-slate-50 text-slate-900 antialiased">
    ...
    <main class="pt-20 pb-12 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto space-y-8">
        @if($accommodation)
        <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 card-hover mb-8">
            ...
            <p class="text-slate-500 mb-4">{{ $accommodation->adress }}</p>

            <!-- 🔹 Local Token Area (ADDED) -->
            <div class="bg-slate-50 border border-slate-200 rounded-xl p-4 mb-4">
                <p class="text-sm text-slate-500 mb-2">Home Invitation Token</p>
                <div class="flex items-center justify-between">
                    <span class="font-mono text-lg tracking-wider text-slate-900">
            ...
            <form id="inviteEmailForm" method="POST" action="{{Route('Email.Sent')}}" 
                ...
                <input type="email" name="email" required placeholder="Enter member email"
                    class="flex-1 px-4 py-2 border border-slate-300 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none transition">
        ...
        <div class="gradient-bg rounded-2xl p-6 text-white relative overflow-hidden shadow-xl">
            ...
            <div class="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                ...
                <div class="flex gap-3">
                    <button onclick="openModal('createColocModal')" class="px-5 py-2.5 bg-white text-indigo-600 rounded-xl font-semibold hover:bg-slate-50 transition shadow-lg">
                        Create Home
                    </button>
        ...
        <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 card-hover mb-8 text-center">
            <p class="text-slate-500">You have no active shared home. Create or join a home to see details here.</p>
        </div>
        @endif
...
@endforeach
    ...
    <footer class="bg-white border-t border-slate-200 px-6 py-6">
        <div class="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-4">
            ...
            <div class="flex items-center gap-6">
                <a href="#" class="text-sm text-slate-500 hover:text-indigo-600 transition">Privacy</a>
                <a href="#" class="text-sm text-slate-500 hover:text-indigo-600 transition">Terms</a>
                <a href="#" class="text-sm text-slate-500 hover:text-indigo-600 transition">Contact</a>
            </div>
        </div>
    </footer>
    ...
    <div id="createColocModal" class="fixed inset-0 z-50 hidden">
        <div class="absolute inset-0 bg-slate-900/50 backdrop-blur-sm transition-opacity" onclick="closeModal('createColocModal')"></div>
        <div class="absolute inset-0 flex items-center justify-center p-4">
            <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg transform transition-all scale-100 max-h-[90vh] overflow-y-auto">
                ...
                <form method="post" action="{{ Route('Create.accommodation')}}" class="p-6 space-y-6" onsubmit="closeModal('createColocModal');">
                    @csrf
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-2">Home Name <span class="text-rose-500">*</span></label>
    ...
    <div id="joinModal" class="fixed inset-0 z-50 hidden">
        <div class="absolute inset-0 bg-slate-900/50 backdrop-blur-sm transition-opacity" onclick="closeModal('joinModal')"></div>
        <div class="absolute inset-0 flex items-center justify-center p-4">
                ...
                <form method="post" action="{{Route('join.home')}}" class="p-6 space-y-6" onsubmit="closeModal('joinModal');">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-2">
                            Invitation Token <span class="text-rose-500">*</span>
                        </label>
                        <input name="token" type="text" placeholder="Enter token" required
                            class="w-full px-4 py-3 border border-slate-300 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-transparent outline-none transition text-center text-lg">
                    ...
                    <div class="flex gap-3">
                        <button type="button" onclick="closeModal('joinModal')"
                            class="flex-1 px-4 py-3 border border-slate-300 text-slate-700 rounded-xl font-medium hover:bg-slate-50 transition">Cancel</button>
                        <button type="submit"
                            class="flex-1 px-4 py-3 bg-emerald-600 text-white rounded-xl font-medium hover:bg-emerald-700 transition shadow-lg">Join</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <div id="expenseModal" class="fixed inset-0 z-50 hidden">
        <div class="absolute inset-0 bg-slate-900/50 backdrop-blur-sm transition-opacity" onclick="closeModal('expenseModal')"></div>
        <div class="absolute inset-0 flex items-center justify-center p-4">
            <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg transform transition-all scale-100">
                ...
                <form method="POST" action="{{Route('add.expenses')}}" class="p-6 space-y-6" onsubmit="closeModal('expenseModal');">
                    ...
                    @endif
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-2">Title <span class="text-rose-500">*</span></label>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm font-medium text-slate-700 mb-2">Amount MAD<span class="text-rose-500">*</span></label>
                            <div class="relative">
                    ...
                    <div class="flex gap-3 pt-2">
                        <button type="button" onclick="closeModal('expenseModal')"
                            class="flex-1 px-4 py-3 border border-slate-300 text-slate-700 rounded-xl font-medium hover:bg-slate-50 transition">Cancel</button>
                        <button type="submit"
                            class="flex-1 px-4 py-3 bg-violet-600 text-white rounded-xl font-medium hover:bg-violet-700 transition shadow-lg">Add</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    @if(session('success'))
    <div id="success-popup" class="fixed top-4 left-1/2 transform -translate-x-1/2 bg-emerald-500 text-white px-6 py-3 rounded-lg shadow-lg z-50 opacity-0 transition-opacity duration-500">
        {{session('success')}}
    </div>
    @endif

    @if(session('failure'))
    <div id="success-popup" class="fixed top-4 left-1/2 transform -translate-x-1/2 bg-rose-500 text-white px-6 py-3 rounded-lg shadow-lg z-50 opacity-0 transition-opacity duration-500">
        {{session('failure')}}
    </div>
    @endif
